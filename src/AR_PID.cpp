...
void analysisFunc(ros::Publisher, ar_drone_ros::traj, Eigen::VectorXd, pid_var);
void spData_Callback(ar_drone_ros::spData); 
void moveFunc(ros::Publisher, Twist, double);
...
int main(int argc, char**argv)
{
	...
	// Create Trackbar 
    cv::namedWindow("PID Control and Destination", CV_WINDOW_NORMAL);
	...
	// Messages 
	Twist control; //Twist  
	ar_drone_ros::traj tdat; //Visual Data Topic 
    ...
    ros::Publisher lmpc_pub   = nh.advertise<ar_drone_ros::traj>("/lmpc_analysis",1);	
...
void moveFunc(ros::Publisher pub, Twist cmd, double t)
{
        // Publish Commands 
		pub.publish(cmd);
        ros::spinOnce();      
...
void analysisFunc(ros::Publisher lmpc_pub, ar_drone_ros::traj traj, Eigen::VectorXd sdat,	pid_var pvar )
{
